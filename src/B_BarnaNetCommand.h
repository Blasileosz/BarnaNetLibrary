...
#define B_ADDRESS_MAP_INVALID_QUEUE ((void*)(-1)) // Returned by B_GetAddress if the TaskID was not found

// ID-Queue key-value pair to translate DEST and FROM values to queues
// Sent to tasks instead of individual queues
typedef struct {
	int queueID;
	unsigned int flags; // Suggests how this task should be routed to
...
#define B_COMMAND_BODY_SIZE (B_COMMAND_STRUCT_SIZE - 4)
...
typedef struct {
	...
	uint8_t transmissionID; // Unique? ID for each command, allows to match requests and responses
...
void B_FillCommandBodyString(B_command_t* const command, size_t offset, const char* const string, size_t maxLength);
inline void B_FillCommandBody_BYTE(B_command_t* const command, size_t offset, uint8_t value) { command->body[offset] = value; }
inline void B_FillCommandBody_WORD(B_command_t* const command, size_t offset, uint16_t value) { *((uint16_t*)&command->body[offset]) = ntohs(value); }
inline void B_FillCommandBody_DWORD(B_command_t* const command, size_t offset, uint32_t value) { *((uint32_t*)&command->body[offset]) = ntohl(value); }
inline uint8_t B_ReadCommandBody_BYTE(B_command_t* const command, size_t offset) { return command->body[offset]; }
inline uint16_t B_ReadCommandBody_WORD(B_command_t* const command, size_t offset) { return htons(*((uint16_t*)(&command->body[offset]))); }
inline uint32_t B_ReadCommandBody_DWORD(B_command_t* const command, size_t offset) { return htonl(*((uint32_t*)(&command->body[offset]))); }
#define B_TASK_FLAG_NO_REPLY 0b1 // Task doesn't need replies (for example, when an alarm triggers, the alarm task doesn't need an OK reply from the led controller)
#define B_TASK_FLAG_ONLY_REPLY 0b10 // Task only accepts replies (for example, the TCP task shouldn't send commands to the MQTT task, only responses to its own commands) used with comms tasks
