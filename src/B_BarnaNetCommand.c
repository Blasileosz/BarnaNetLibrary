...
{
	if (command == NULL || string == NULL || strlen(string) + offset + 1 > B_COMMAND_BODY_SIZE)
		return;
	
	strncpy((char*)&command->body[offset], string, maxLength - 1);
}
...
bool B_SendStatusReply(B_addressMap_t* const addressMap, const B_command_t* const command, uint8_t thisTaskID, uint8_t commandOP, const char* const string) {
	...
	B_FillCommandBodyString(&response, 0, string, B_COMMAND_BODY_SIZE);
...
bool B_InsertAddress(B_addressMap_t* const addressMap, int insertIndex, int taskID, QueueHandle_t queueHandle, unsigned int flags)
{
	if (addressMap == NULL || addressMap->addressList == NULL || taskID == 0 || queueHandle == NULL)
		return false;
	...
	addressMap->addressList[insertIndex].queueID = taskID;
...
QueueHandle_t B_GetAddress(const B_addressMap_t* const addressMap, int taskID)
{
	if (addressMap == NULL || addressMap->addressList == NULL || taskID == 0)
		return B_ADDRESS_MAP_INVALID_QUEUE;

	for (int i = 0; i < addressMap->mapSize; i++)
	{
		if (addressMap->addressList[i].queueID == taskID)
...
// Returns B_ADDRESS_MAP_INVALID_QUEUE if queue with the given TaskID was not found
QueueHandle_t B_GetAddressAndFlags(const B_addressMap_t* const addressMap, int taskID, unsigned int* const flagsOut)
{
	if (addressMap == NULL || addressMap->addressList == NULL || taskID == 0 || flagsOut == NULL)
		return B_ADDRESS_MAP_INVALID_QUEUE;
